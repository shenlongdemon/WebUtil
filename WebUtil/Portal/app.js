(function () {
    ...
    var app = angular
            .module('app', ['ngRoute', 'ngCookies', 'ui.router', 'ngAnimate', 'cfp.loadingBar', 'smart-table', 'ngStorage', 'ui.bootstrap', 'ngCsv', 'dateParser', 'darthwade.dwLoading', 'angularMoment', 'cfp.hotkeys']);
    ...
    app.config(config)
            .constant('Constants',
                    {
                        API_BASE: "http://localhost:49709/api"
                    }
            )
            .run(run);
    ...
    function config($routeProvider, $httpProvider, cfpLoadingBarProvider, $provide, $compileProvider) {
        ...
        $routeProvider
                ...
                .when('/children', {
                    ...
                    templateUrl: 'view/children.view.html',
                    controllerAs: 'vm'
                })
    ...
    function run($rootScope, $location, $cookieStore, $http, $injector, $localStorage, HttpPendingRequestsService) {
        $rootScope.version = currentVersion; // "1.0.77";
        ...
        $injector.get("$http").defaults.transformRequest = function (data, headersGetter) {
            ...
            headersGetter()['Authorization'] = access_token;// window.localStorage.getItem('access_token');
            if (data) {
